          ...
          <div class="row">
            <div class="col-md-3 col-sm-6 col-12">
              <div class="info-box">
                ...
                <?php
                // Ensure the room_id and date are set in GET parameters
                if (isset($_GET['room_id']) && isset($_GET['date'])) {
                  $room_id = intval($_GET['room_id']); // Sanitize room_id
                  $year = intval($_GET['date']); // Get the year from the date parameter
                } else {
                  // Fallback values if parameters are not set
                  $room_id = 0; // Default or error handling
                  $year = date('Y'); // Use current year if date is not set
                }

                // Query to get the total electricity bill
                $query = mysqli_query($conn, "SELECT SUM(amount) AS total_bill FROM room_bill WHERE room_id = $room_id AND billtype = 'Electricity' AND YEAR(date) = $year");
                if ($query) {
            ...
            <div class="col-md-3 col-sm-6 col-12">
              <div class="info-box">
                ...
                <?php
                // Query to get the total water bill
                $query = mysqli_query($conn, "SELECT SUM(amount) AS total_water FROM room_bill WHERE room_id = $room_id AND billtype = 'Water' AND YEAR(date) = $year");
                if ($query) {
                  $result = mysqli_fetch_assoc($query);
                        ...
                        <tbody>
                          ...
                          if (isset($_GET['room_id']) && isset($_GET['date'])) {
                            ...
                            $query = mysqli_query($conn, "
                              ...
                              WHERE room_bill.room_id = $room_id 
                                AND billtype = 'Electricity' 
                                AND YEAR(date) = '$date' 
                              ORDER BY date DESC
                            ");
