...
<body class="hold-transition sidebar-mini">
                  ...
                  <table id="example1" class="table table-striped">
                    ...
                    <tbody>
                      ...
                      $query = mysqli_query(
                        $conn,
                        "SELECT *, 
                                (SELECT COUNT(DISTINCT tenants.tenants_id) 
                                FROM rent 
                                INNER JOIN tenants ON tenants.tenants_id = rent.tenants_id 
                                WHERE rent.room_id = room.room_id) AS tenant_count
                              FROM `room`
                               WHERE room_status IS NULL
                              ORDER BY 
                                CASE 
                                  WHEN (maximum - (SELECT COUNT(DISTINCT tenants.tenants_id) 
                                                  FROM rent 
                                                  INNER JOIN tenants ON tenants.tenants_id = rent.tenants_id 
                                                  WHERE rent.room_id = room.room_id)) > 0 THEN 1
                                  ELSE 2
                                END, room_id DESC"
                      );
                      ...
                      while ($result = mysqli_fetch_array($query)) {
                        ...
                        $queryCount = mysqli_query($conn, "SELECT COUNT(DISTINCT tenants.tenants_id) AS tenant_count 
                           FROM rent 
                           INNER JOIN tenants ON tenants.tenants_id = rent.tenants_id 
                           WHERE rent.room_id = $room_id AND tenants.status = 1 AND rent.status = 'Unpaid'");
                        ...
                        $queryJoin = mysqli_query($conn, "SELECT DISTINCT tenants.profile, tenants.name 
                          FROM rent 
                          INNER JOIN tenants ON tenants.tenants_id = rent.tenants_id 
                          WHERE rent.room_id = $room_id AND tenants.status = 1 AND rent.status = 'Unpaid' ");
                        ...
                        <form id="assigntenantForm" enctype="multipart/form-data" action="../inc/controller.php" class="form-horizontal">
                          <div class="modal-body" style="padding-top:0; padding-bottom:0;">
                            <div class="card-body">
                              ...
                              <div class="form-group row">
                                ...
                                <div class="col-sm-9">
                                  <select id="selectname" class="form-control select2bs4" data-placeholder="Select Name" required name="tenanupdateID" style="width: 100%;">
                                    ...
                                    $query = mysqli_query($conn, "SELECT * FROM `tenants` WHERE `status` = 0");
                                    while ($result = mysqli_fetch_array($query)) {
                                      extract($result);
                                    ?>
                              ...
                              <div class="form-group row">
                                ...
                                <div class="col-sm-9">
                                  <select id="selectRoom" class="form-control select2bs4" data-placeholder="Select Room" required name="room_id" style="width: 100%;">
                                    ...
                                    $query = mysqli_query(
                                      $conn,
                                      "SELECT room.*, (room.maximum - COALESCE((SELECT COUNT(DISTINCT tenants.tenants_id)  FROM rent INNER JOIN tenants ON tenants.tenants_id = rent.tenants_id  WHERE rent.room_id = room.room_id), 0)) AS free_beds FROM `room`  WHERE room_status IS NULL  HAVING free_beds > 0"
                                    );
                              ...
                              <!-- <div class="form-group row">
                                <label class="col-sm-3 col-form-label">Rental Payment </label>
                                <div class="col-sm-9">
                                  <input type="number" class="form-control" name="payment" placeholder="Rental Payment " required>
                                </div>
                              </div> -->
                              <div class="form-group row">
 ...
 $('input[name="depositMonths"]').on('input', function () {
                // Get the values of monthly rate and deposit months
                const monthlyRate = parseFloat($('input[name="roomsmonthly"]').val()) || 0;
                const depositMonths = parseFloat($(this).val());

                if (!depositMonths) {
                    // If depositMonths is empty or invalid, clear the deposit field
